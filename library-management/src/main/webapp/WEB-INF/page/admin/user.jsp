    ...
    <div class="row">
        <div class="col-md-12">
            ...
            <table id="dg" title="用户列表" iconCls="icon-man" class="easyui-datagrid" width="100%"
                ...
                <th field="state" width="100" align="center" formatter="formatUserState">状态</th>
            ...
            <div id="dlg" class="easyui-dialog" style="width:400px"
                ...
                <form id="fm" novalidate style="margin:0;padding:20px 50px">
                    ...
                    <div style="margin-bottom:10px">
                        <input name="realName" id="n_userTrueName" class="easyui-textbox" required="true"
                               label="真实姓名:"
                               style="width:100%">
                    </div>
                    <div style="margin-bottom:10px">
            ...
            <div id="dlg1" class="easyui-dialog" style="width:400px"
                ...
                <form id="fm1" novalidate style="margin:0;padding:20px 50px">
                    ...
                    <div style="margin-bottom:10px">
                        <input name="realName" id="u_userTrueName" class="easyui-textbox" required="true"
                               label="真实姓名:"
                               style="width:100%">
                    </div>
...
<script>
    ...
    function formatOperate(value, row) {
        ...
        if(row.state == 1){//启用
            return "<button onclick='disableUser(" + row.id + ")' class='btn-warning'>禁用</button> <button onclick=\"resetUserPassword('" + row.id + "','" + row.userEmail + "')\" class='btn-danger'>重置密码</button> <button onclick=\"openRoleChooseDialog('" + row.roles + "','" + row.id + "')\" class='btn-info'>分配角色</button>  ";
        }else {//禁用
            return "<button onclick='enableUser(" + row.id + ")' class='btn-success'>启用</button>";
        }
    }
    ...
    function openUserModifyDialog() {
        ...
        $("#fm1").form("load", row);
        //赋值url
        url = "<%=basePath%>/user/update?id=" + row.id;
    }
    ...
    function deleteUser() {
        ...
        $.messager.confirm("系统提示", "您确定要删除这条数据吗?", function (r) {
            if (r) {
                $.ajax({
                    type: "DELETE",
                    url: "<%=basePath%>/user/delete?id=" + userId,
                    // headers: {"token": $.cookie("token")},
                    success: function (res) {
                        if (res.ret) {
    ...
    $(document).ready(function () {
        ...
        $("#dg").datagrid({
            onDblClickRow: function (index, row) {
                $("#dlg1").dialog("open").dialog("center").dialog("setTitle", "修改用户信息");
                $("#fm1").form("load", row);
                url = "<%=basePath%>/user/update?userId=" + row.userId;
            }
        });
    ...
    function saveRoleSet() {
        ...
        console.log(roleIds);
        $.post("<%=basePath%>/user/saveRoleSet", {roleIds: roleIds, userId: userId}, function (res) {
            if (res.ret) {
